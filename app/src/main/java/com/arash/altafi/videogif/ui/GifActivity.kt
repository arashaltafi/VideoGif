...
import com.arash.altafi.videogif.utils.CustomMenuItemView
import com.arash.altafi.videogif.App
import com.arash.altafi.videogif.utils.MyCropParams
...
import com.arash.altafi.videogif.utils.MyConstants.DOUBLE_BACK_TO_EXIT_DELAY
import com.arash.altafi.videogif.utils.MyConstants.EXTRA_CROP_PARAMS
import com.arash.altafi.videogif.utils.MyConstants.EXTRA_TRIM_END
import com.arash.altafi.videogif.utils.MyConstants.EXTRA_TRIM_START
import com.arash.altafi.videogif.utils.MyConstants.EXTRA_VIDEO_URI
import com.arash.altafi.videogif.utils.MyConstants.FFMPEG_COMMAND_FOR_ALL
import com.arash.altafi.videogif.utils.MyConstants.FIRST_FRAME_PATH
import com.arash.altafi.videogif.utils.MyConstants.GIF_COLOR_QUALITY_MAP
import com.arash.altafi.videogif.utils.MyConstants.GIF_FRAME_RATE_MAP
import com.arash.altafi.videogif.utils.MyConstants.GIF_RESOLUTION_MAP
import com.arash.altafi.videogif.utils.MyConstants.GIF_SPEED_GLANCE_MODE
import com.arash.altafi.videogif.utils.MyConstants.GIF_SPEED_MAP
import com.arash.altafi.videogif.utils.MyConstants.MATERIAL_TOOLBAR_SUBTITLE_TEMP_DISPLAY_DURATION
import com.arash.altafi.videogif.utils.MyConstants.PALETTE_PATH
import com.arash.altafi.videogif.utils.MyConstants.THUMBNAIL_PATH
import com.arash.altafi.videogif.utils.MyConstants.UNKNOWN_INT
import com.arash.altafi.videogif.utils.MySettings.INT_PREVIOUS_GIF_CONFIG_UNKNOWN_VALUE
import com.arash.altafi.videogif.databinding.ActivityGifBinding
import com.arash.altafi.videogif.utils.MySettings
import com.arash.altafi.videogif.utils.MyToolbox
...
class GifActivity : AppCompatActivity() {
    ...
    private var trimTimeStart = UNKNOWN_INT
    private var trimTimeEnd = UNKNOWN_INT
    ...
    private val getCropAndTrimResult =
    ...
    private val requestPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
            Toast.makeText(
                this@GifActivity, when (isGranted) {
                    true -> {
    ...
    private val videoKeyFramesTimestampList by lazy {
        val list =
            FFprobeKit.execute("-loglevel error -skip_frame nokey -select_streams v:0 -show_entries frame=pts_time -of csv=p=0:sv=fail ${inputVideoSaf()}").allLogsAsString.split(
                "\n"
            ).map {
                try {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        inputVideoUri = intent.getParcelableExtra(EXTRA_VIDEO_URI) ?: intent.getParcelableExtra(
            Intent.EXTRA_STREAM
        ...
        videoInformationSession =
            FFprobeKit.getMediaInformationFromCommand("-v quiet -hide_banner -print_format json -show_format -show_streams -show_chapters -i " + inputVideoSaf())
        ...
        binding.chipCrop.setOnClickListener {
            getCropAndTrimResult.launch(
                CropActivity.startIntent(
                    this@GifActivity,
                    myCropParams,
                    defaultCropParams,
                    inputVideoUri,
    ...
    private fun setDefaultCropParams() {
        try {
            val videoStreamInfo =
                videoInformationSession.mediaInformation.streams.first { it.type == "video" }
            trimTimeStart = 0
            trimTimeEnd = inputVideoDuration()
            var rotation =
                try {
                    -((videoStreamInfo.allProperties.get("side_data_list") as JSONArray).get(0) as JSONObject).getInt(
                        "rotation"
                    )
                } catch (e: Exception) {
                    0
    ...
    private fun loadCustomMenuItemView() {
        ...
        cmivResolution.setUpWithDropDownConfig(
            GIF_RESOLUTION_MAP,
            CUSTOM_MENU_ITEM_VIEW_TYPE_GIF_CHIPS
        )
        cmivFrameRate.setUpWithDropDownConfig(
            GIF_FRAME_RATE_MAP,
            CUSTOM_MENU_ITEM_VIEW_TYPE_GIF_CHIPS
        )
        cmivColorQuality.setUpWithDropDownConfig(
            GIF_COLOR_QUALITY_MAP,
            CUSTOM_MENU_ITEM_VIEW_TYPE_GIF_CHIPS
        )
        ...
        when (MySettings.rememberGifOptions) {
            true -> {
                ...
                if (previousGifConfigSpeed != INT_PREVIOUS_GIF_CONFIG_UNKNOWN_VALUE &&
                    ...
                    materialToolbarSubtitle.text =
                        getString(R.string.previously_saved_options_loaded)
    ...
    private fun loadFirstFrame() {
        ...
        try {
            ...
            val commandLoadFirstFrame =
                "$FFMPEG_COMMAND_FOR_ALL -ss ${trimTimeStart}ms -i ${inputVideoSaf()} -frames:v 1 -q:v 10 -y $FIRST_FRAME_PATH"
    ...
    private fun loadCroppedAndTrimmedThumbnail() {
        ...
        try {
            val commandLoadCroppedAndTrimmedThumbnail =
                "$FFMPEG_COMMAND_FOR_ALL -i $FIRST_FRAME_PATH -vf ${cropParams()}${transposeParams()} -q:v 10 -y $THUMBNAIL_PATH"
            MyToolbox.logging(
                "command",
                "loadCroppedAndTrimmedThumbnail: $commandLoadCroppedAndTrimmedThumbnail"
            )
    ...
    private fun loadVideoFailed() {
        runOnUiThread {
            Toast.makeText(
                this@GifActivity,
                getString(R.string.load_video_failed),
                Toast.LENGTH_LONG
            ).show()
            finishAndRemoveTask()
            keepScreenOn(false)
    ...
    private fun conversionSuccessfully() {
        ...
        runOnUiThread {
            ...
            materialToolbar.subtitle = getString(
                R.string.gif_saved_s_mb,
                MyToolbox.keepNDecimalPlaces(MyToolbox.getFileSizeFromUri(gifUri) / 1048576.0, 2)
            )
            ...
            binding.mbDeleteAndRedo.setOnClickListener {
                ...
                materialToolbar.subtitle =
                    getString(R.string.the_gif_just_converted_has_been_deleted)
                Handler(Looper.getMainLooper()).postDelayed({
    ...
    private fun createNewGifFileAndReturnUri(): Uri {
        val fileName = "${
            MyToolbox.getFileNameFromUri(
                inputVideoUri,
                true
            )
        }_VideoToGif_${MyToolbox.getTimeYMDHMS()}.gif"
        return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            val dir = "${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)}/VideoToGif/"
            ...
            App.context.contentResolver.insert(
                MediaStore.Images.Media.getContentUri(
                    ...
                    put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/VideoToGif")
        ...
        thread {
            ...
            val command1Vf =
                when (analyzeVideoSlowly) {
                    false -> "${cropParams()}${transposeParams()},scale=${
                        resolutionParams(
                            GIF_RESOLUTION_MAP.values.min()
                        )
                    }:flags=fast_bilinear,palettegen=max_colors=${cmivColorQuality.selectedValue()}:stats_mode=diff"
            ...
            val command1 =
                if (command1Vf == command1VfCached && trimTimeCached.first == trimTimeStart && trimTimeCached.second == trimTimeEnd) {
                    "-version" // do nothing
                } else {
                    when (analyzeVideoSlowly && cmivSpeed.selectedValue() != GIF_SPEED_GLANCE_MODE) {
                        false -> {
                            ...
                            val trimTimeKeyFrameStart =
                                videoKeyFramesTimestampList.filter { it < trimTimeStart }
                                    .maxOrNull() ?: 0
                            val trimTimeKeyFrameEnd =
                                videoKeyFramesTimestampList.filter { it > trimTimeEnd }.minOrNull()
                                    ?: inputVideoDuration()
                    ...
                    it.returnCode.isValueSuccess -> {
                        ...
                        val command2 = when (cmivSpeed.selectedValue()) {
                            GIF_SPEED_GLANCE_MODE -> {
                                ...
                                outputFramesEstimated =
                                    ceil(videoKeyFramesTimestampInTrimmedCount().toDouble() / command2FrameStep).toInt()
                                ...
                                outputFramesEstimated =
                                    ceil((trimTimeEnd - trimTimeStart) * outputFpsTarget / outputSpeed / 1000f).toInt()
                        ...
                        FFmpegKit.executeAsync(command2, { ffmpegSession ->
                            ...
                            when {
                                ...
                                ffmpegSession.returnCode.isValueError -> conversionUnsuccessfully(
                            ...
                            runOnUiThread {
                                ...
                                progress =
                                    min((it.videoFrameNumber * 100 / outputFramesEstimated), 99)
                                linearProgressIndicator.setProgress(progress, true)
                                materialToolbar.subtitle = getString(
                                    R.string.converting_s_s_mb,
                                    progress,
                                    MyToolbox.keepNDecimalPlaces(it.size / 1048576.0, 2)
                                )
                            }
                        })
                    }
                    it.returnCode.isValueError -> conversionUnsuccessfully(
                        canceledByUser = false,
                        finishActivity = true
                    )
                }
    ...
    private fun onConvertClick() {
        ...
        if (converting) {
            ...
            if ((Build.VERSION.SDK_INT < Build.VERSION_CODES.Q
                ...
                updateConvertingState()
                materialToolbarSubtitle.text =
                    getString(R.string.you_need_to_allow_storage_permission_to_save_your_gif)
    ...
    private fun cropParams() =
        "crop=${myCropParams.outW}:${myCropParams.outH}:${myCropParams.x}:${myCropParams.y}"
    ...
    override fun onBackPressed() {
        ...
        Toast.makeText(
            this@GifActivity,
            getString(R.string.press_back_again_to_quit),
            Toast.LENGTH_SHORT
        ).show()
        Handler(Looper.getMainLooper()).postDelayed(
            { doubleBackToExitPressedOnce = false },
            DOUBLE_BACK_TO_EXIT_DELAY
        )
    }
    ...
    companion object {
        fun start(context: Context, videoUri: Uri) {
            context.startActivity(
                Intent(context, GifActivity::class.java).putExtra(
                    EXTRA_VIDEO_URI,
                    videoUri
                )
            )
